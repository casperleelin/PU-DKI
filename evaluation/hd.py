...
sparse_path = "real_rock_outcrop/Real/volcanic.xyz"
upsampled_path = "real_rock_outcrop/Real_up_PUCRN/volcanic.xyz"
cd_dist_compute = CD_dist()
pcd = o3d.io.read_point_cloud(sparse_path)
pcd_name = sparse_path.split('/')[-1]
gt = torch.Tensor(np.asarray(o3d.io.read_point_cloud(upsampled_path).points)).unsqueeze(0).cuda()
input_pcd = np.array(pcd.points)
input_pcd = torch.from_numpy(input_pcd).float().cuda()
pred = input_pcd.unsqueeze(0)
