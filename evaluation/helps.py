...
def reliable_gpu_fps(points, n_samples):
    """reliable GPU FPS"""
...
def torch_cluster_fps(points, n_samples):
    """
    使用torch-cluster进行FPS - 专门为大规模点云优化
    """
    # points: (B, N, 3)
    B, N, C = points.shape
    
    # 将批量数据转换为torch-cluster格式
    points_flat = points.reshape(-1, 3)  # (B*N, 3)
    batch = torch.cat([torch.full((N,), i, dtype=torch.long) for i in range(B)]).to(points.device)
    
    # 计算采样比例
    ratio = n_samples / N
    
    # 执行FPS
    indices = fps(points_flat, batch, ratio=ratio, random_start=False)
    
    # 重新组织为批量格式
    sampled_batch = batch[indices]
    unique_batches, counts = torch.unique(sampled_batch, return_counts=True)
    
    # 确保每个批次都有正确的点数
    final_indices = []
    for i in range(B):
        mask = sampled_batch == i
        batch_indices = indices[mask]
        # 调整索引到批次内的位置
        batch_indices = batch_indices - i * N
        final_indices.append(batch_indices[:n_samples])  # 确保不超过目标点数
    
    return torch.stack(final_indices)

def reliable_FPS(pts, fps_pts_num):
    # input: (b, 3, n)

    # (b, n, 3)
    pts_trans = rearrange(pts, 'b c n -> b n c').contiguous()
    # (b, fps_pts_num)
    sample_idx = torch_cluster_fps(pts_trans, fps_pts_num).long()
    # (b, 3, fps_pts_num)
    sample_pts = index_points(pts, sample_idx)
...
def add_noise(pts, sigma, clamp):
    assert clamp > 0
    noise = np.random.randn(*pts.shape) * sigma
    
    noise = np.clip(noise, -clamp, clamp)

    jittered_data = pts + noise
...
def add_gaussian_noise(point_cloud, mean=0, std=0.01):
    noise = torch.randn_like(point_cloud) * std + mean
