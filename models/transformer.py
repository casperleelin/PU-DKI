...
from models.helps import get_knn_pts, index_points
from einops import repeat, rearrange

class Encoder_Attention(nn.Module):
    def __init__(self, args, geo_dim=3):
        super().__init__()
        self.k = args.k
        
        self.q_conv = nn.Conv1d(args.encoder_dim, args.encoder_dim, 1)
        self.k_conv = nn.Conv1d(args.encoder_dim, args.encoder_dim, 1)
        self.v_conv = nn.Conv1d(args.encoder_dim, args.encoder_dim, 1)
        self.geo_mlp = nn.Sequential(
            nn.Conv2d(geo_dim, args.encoder_dim//2, 1, 1),
            nn.BatchNorm2d(args.encoder_dim//2) if args.encoder_bn else nn.Identity(),
            nn.ReLU(inplace=True),
            nn.Conv2d(args.encoder_dim//2, args.encoder_dim, 1, 1)
        )
        self.rel_mlp = nn.Sequential(
            nn.Conv2d(args.encoder_dim, args.encoder_dim//2, 1, 1),
            nn.BatchNorm2d(args.encoder_dim//2),
            nn.ReLU(inplace=True),
            nn.Conv2d(args.encoder_dim//2, args.encoder_dim, 1, 1)
        )
        # self.attn_conv = nn.Conv1d(cfgs.encoder_dim, cfgs.encoder_dim, 1)
        self.out_conv = nn.Sequential(
            nn.Conv1d(args.encoder_dim, args.encoder_dim*2, 1, 1),
            nn.BatchNorm1d(args.encoder_dim*2) if args.encoder_bn else nn.Identity(),
            nn.ReLU(inplace=True),
            nn.Conv1d(args.encoder_dim*2, args.encoder_dim, 1, 1),
            nn.BatchNorm1d(args.encoder_dim) if args.encoder_bn else nn.Identity()
        )
